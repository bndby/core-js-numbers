...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const vectorsScalarMul = x1 * x2 + y1 * y2;
  const vectorsValues =
    Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);

  return Math.acos(vectorsScalarMul / vectorsValues);
}
...
function getSumOfNumbers(x1, x2, x3) {
  const decPlaces = (num) => {
    ...
    return strNum.includes('.') ? strNum.split('.')[1].length : 0;
  };

  const maxDecPlaces = Math.max(decPlaces(x1), decPlaces(x2), decPlaces(x3));
  const multiplier = 10 ** maxDecPlaces;
  return (x1 * multiplier + x2 * multiplier + x3 * multiplier) / multiplier;
}
